               ...
               va_list    ap)		/* I - Pointer to additional arguments */
{
  ...
  */
  ...
  while (1)
  {
      va_copy(apcopy, ap); /* Create a copy in each iteration to avoid args being eaten by vsnprintf */
      bytes = vsnprintf(buffer, size, format, apcopy);
      va_end(apcopy);      /* Doc says to end copies (possibly preventing memory leaks) */

      if ((size_t)bytes < (size_t)size)
      {
          /*
           * Hey, the formatted string fits in the tiny buffer, so just return that...
           */
          return buffer;
      }
